...
class TestMergeCatalog:
    ...
    def test_all_new_lots(self, mock_fetch, mock_delete, mock_create, db):
    ...
    def test_all_unchanged(self, mock_fetch, mock_delete, mock_create, db):
    ...
    @patch("catalog.services.delete_lot")
    @patch("catalog.services.fetch_all_lots")
    def test_mixed_scenario(self, mock_fetch, mock_delete, mock_create, db):
    ...
    @patch("catalog.services.delete_lot")
    @patch("catalog.services.fetch_all_lots")
    def test_override_preservation(self, mock_fetch, mock_delete, mock_create, db):
        ...
        # LotDataDto uses Qty/L aliases but Python attrs are qty/l
    ...
    @patch("catalog.services.delete_lot")
    @patch("catalog.services.fetch_all_lots", return_value=[])
    def test_best_effort_failure(self, mock_fetch, mock_delete, mock_create, db):
    ...
    @patch("catalog.services.delete_lot")
    @patch("catalog.services.fetch_all_lots", return_value=[])
    def test_duplicate_customer_item_id_dedup(self, mock_fetch, mock_delete, mock_create, db):
        ...
        # Verify the first occurrence was used (qty=1, not qty=99)
